 ...
 */
public class WebRadioGroup extends Webelement implements RadioGroup {
     ...
     */
    public WebRadioGroup(final WebElement element) {
        super(element);
    ...
    public WebRadioGroup(OrasiDriver driver, By by) {
        super(driver, by);
        logTrace("Entering RadioGroupImpl#init");
    ...
    public void selectByOption(String option) {
        ...
        for (int loopCounter = 0; loopCounter < stringOptions.size(); loopCounter++) {
            if (stringOptions.get(loopCounter).trim().equalsIgnoreCase(option.trim())) {
                ...
                try {
                    new Webelement(getWrappedDriver(), By.xpath("//input[" + (currentIndex + 1) + "]")).click();
                } catch (RuntimeException rte) {
